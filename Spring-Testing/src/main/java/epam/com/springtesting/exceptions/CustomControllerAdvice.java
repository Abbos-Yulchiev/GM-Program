...
public class CustomControllerAdvice extends ResponseEntityExceptionHandler implements ResponseErrorHandler {
    ...
    public ResponseEntity<ApiExceptionMessage> illegalArgumentException(InsufficientAmountException e) {
        return new ResponseEntity<>(new ApiExceptionMessage(e.getMessage(), HttpStatus.CONFLICT, LocalDateTime.now()), HttpStatus.NOT_FOUND);
    }
